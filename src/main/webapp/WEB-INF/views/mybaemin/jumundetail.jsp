   ...
   <table width="600" align="center">
      ...
      <tr>
         <td colspan="3" align="left">
            <b style="color:#2ac1bc;font-size:18px;">${ map.state2 }</b>
         </td>
      </tr>
      <tr height="50">
         <td colspan="3" align="left">
            <b style="font-size:22px;color:black;">${ tmap.get(0).shop }</b> <p>
         </td>
      </tr>
      <c:forEach items="${tmap}" var="map">
      <tr>
         <td colspan="3" align="left">
            <span style="font-size:18px;">${ map.menu } ${ map.su }개</span>
         </td>
      </tr>
      <tr>
         <td colspan="3" align="left">
      ...
      <tr>
         <td colspan="3" align="center">
            ...
            <c:if test="${tmap.get(0).state2==0}">
               <c:set var="state2" value="결제가 완료되었어요" />
            </c:if>
            <c:if test="${tmap.get(0).state2==1}">
               <c:set var="state2" value="음식 준비 중이에요" />
            </c:if>
      
            <!-- 포장일 때 -->
            <c:if test="${map.baeprice==0}">
               <c:if test="${tmap.get(0).state2==2}">
                  <c:set var="state2" value="음식이 준비 완료되었어요" />
               </c:if>
               <c:if test="${tmap.get(0).state2==3}">
                  <c:set var="state2" value="픽업이 완료되었어요" />
               </c:if>
            </c:if>
            ...
            <c:if test="${map.baeprice!=0}">
               <c:if test="${tmap.get(0).state2==2}">
                  <c:set var="state2" value="배달 중이에요" />
               </c:if>
               <c:if test="${tmap.get(0).state2==3}">
                  <c:set var="state2" value="배달이 완료되었어요" />
               </c:if>
            </c:if>
      
            <c:if test="${tmap.get(0).state2==4}">
               <c:set var="state2" value="취소 진행 중이에요" />
            </c:if>
            <c:if test="${tmap.get(0).state2==5}">
               <c:set var="state2" value="취소가 완료되었어요" />
            </c:if>
            
            <!-- 결제 완료일 때 -->
            <c:if test="${ tmap.get(0).state2 == 0}">
                  <input type="button" onclick="location='chgstate?state2=4&jumuncode=${tmap.get(0).jumuncode}'" value="주문 취소하기">
            </c:if>
            
            <!-- 주문취소 승인 됐을 때 -->
            <c:if test="${ tmap.get(0).state2 == 5}">
                  <input type="button" value="주문 취소완료">
            </c:if>
                    
            <!-- 픽업, 배달 완료되었을 때 -->   
            <c:if test="${ tmap.get(0).state2 == 3 }">
	            <c:if test="${tmap.get(0).hugi == 0}">
	            	<input type="button" onclick="location='review?guid=${tmap.get(0).id}&jumuncode=${tmap.get(0).jumuncode}'" value="후기 작성하기">
				</c:if>
	            <c:if test="${tmap.get(0).hugi == 1}">
	            	<input type="button" value="후기 작성완료">
	           	</c:if> 
           </c:if>  
           <input type="button" onclick="location='../food/fcontent?fcode=${ tmap.get(0).fcode }'" value="가게보기">
         </td>
      </tr>
   </table>
